...
[Here, we will use the entire dataset for training and ignore $K$-fold cross-validation to illustrate the concepts.]{style="color:darkred;"}
...
Therefore, assessing the amount of the uncertainty in these estimates is important. To this end, we use [hypothesis]{style="color:blue;"} tests on individual coefficients.
...
::: incremental
-   $\beta_j$ is the [**true (unknown)**]{style="color:darkblue;"} coefficient of the predictor.
...
::: incremental
1.  Take a random sample ([**your training data**]{style="color:darkblue;"}).
...
[As mentioned before, we will use the entire dataset for training.]{style="color:darkred;"}
...
An issue with **statsmodels** is that we must add the intercept to our predictor matrix using the `add_constant()` function.
...
Since the p-value is smaller than $\alpha = 0.05$, we reject $H_0$ for acceleration and weight. Therefore, [**weight and acceleration have a significant association with the mpg of a car**]{style="color:darkgreen;"}.
...
2.  For any value of the predictor $X_j$, the dispersion or variance is roughly the same.
3.  The $\epsilon_i$’s are independent from each other.
4.  [The $\hat{\epsilon}_i$’s follow normal distribution.]{style="color:gray;"}
:::

. . .

To evaluate the assumption of a linear regression model, we use a [**Residual analysis**]{style="color:green;"}.
...
The [**residuals**]{style="color:purple;"} of this estimated model are $\hat{\epsilon}_i = Y_i - \hat{Y}_i$.
...
2.  Next, calculate the [**theoretical**]{style="color:pink;"} percentiles of a (standard) Normal distribution using Python.
...
1.  [If the model structure is incorrect]{style="color:red;"}, the estimated coefficients $\hat{\beta}_j$ will be incorrect and the predictions $\hat{Y}_i$ will be inaccurate.
