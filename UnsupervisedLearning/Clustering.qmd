...
-   [The solution is to **standardize** the units of the predictors.]{style="color:darkred;"}
...
sns.scatterplot(data = clustered_penguins, x = 'bill_length_mm', y = 'bill_depth_mm', hue = 'Cluster', palette = 'Set1')
plt.title('K-means Clustering of Penguins')
plt.xlabel('Bill Length (mm)')
plt.ylabel('Bill Depth (mm)')
plt.tight_layout()
...
Unfortunately, the previous functions do not allow us to easily extract the cluster assignments. To overcome this issue, we use the function `fclusters` from the **scipy** library in Python.
...
# Extract the clusters using fcluster
distance_threshold = Clust_Cereal[-(num_clusters), 2]
cereal_clusters = fcluster(Clust_Cereal, distance_threshold, criterion='distance')

# Display the clusters
print(cereal_clusters)
```

> In contrast with the creation of the dendrogram, here we use `-(num_clusters)` insetad of `-(num_clusters - 1)`
...
# Add new variable with cluster assignment.
X_cereal_clust = (X_cereal
                  .assign(Clusters = cereal_clusters)
                  )

# Preview the new variable.
X_cereal_clust.head(4)
```
